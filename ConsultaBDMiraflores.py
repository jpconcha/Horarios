...
def get_key(my_dict,val): 
    ...
    return "no existe"
def ConsultaDisponibilidadSalas(ConsultaDía,ConsultaPeriodo):
    ...
    PabellonesMiraflores = BDMiraflores['Pabellón'].drop_duplicates().dropna(axis=0,how='any')
    ...
    salasDisponibles={}
    for pabellon in PabellonesMiraflores:
        salasDisponibles[pabellon] = []    
    for s in SalasMiraflores:
        if s not in Consulta.values:
            filtro = BDMiraflores['Sala'] == s
            sala_pabellon = BDMiraflores.loc[filtro, 'Pabellón'].values[0]
            salasDisponibles[sala_pabellon].append(s)
...
def ConsultaBDHorario():    
    ...
    while op1=='S':
        ...
        for s in SalasMiraflores:
            if s not in Consulta.values:
                #salasDisponibles['pabellon'].append(s)
                # Filtrar el DataFrame por la sala específica
                filtro = BDMiraflores['Sala'] == s
                sala_pabellon = BDMiraflores.loc[filtro, 'Pabellón'].values[0]
                salasDisponibles[sala_pabellon].append(s)
                if s=='9202':
                    ...
                    print('    '+s+'-----'+ str(sala_pabellon))    
        print(' ')
        print(salasDisponibles)
        print('--------------------------------------------------------')
        op1=input('Desea consultar nuevamente:(S/N) ')
